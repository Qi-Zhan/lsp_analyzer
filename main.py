...
class LanguageServerAnalyzer:
    def __init__(self, language: Language, repository_root_path: str):
        ...
        self.lsp = SyncLanguageServer.create(config, logger, repository_root_path)
    ...
    def request_definition_by_line(
        ...
        if lsp_results is None:
            return None
        query = self.tree_sitter_language.query("""(identifier)@element""")
        for lsp_result in lsp_results:
            ...
            first = filter(
                lambda ident: pos_eq(lsp_range, ident.range), all_identifiers
            )
    ...
    def request_definition(
        self, file: str, node: tree_sitter.Node
    ) -> tuple[str, tree_sitter.Node] | None:
    ...
    def request_rename_by_line(
        self, file: str, line: int, column: int, new_name: str
    ) -> bool:
        ...
        for change in document_changes:
            ...
            new_lines = (
                self.workspace.get_by_url(url).root_node.text.decode().splitlines()
            )
            edits = change["edits"]
            for edit in edits:
                range_ = edit["range"]
                ...
                new_lines[line] = (
                    new_lines[line][0 : start["character"]]
                    + new_text
                    + new_lines[line][end["character"] :]
                )
            self.workspace.set_by_url(
                url, self.workspace.parser.parse("\n".join(new_lines).encode())
            )
        return True
    ...
    def get_text(self, relative_path: str) -> str:
        return self.workspace.get_by_relative_path(
            relative_path
        ).root_node.text.decode()


def pos_eq(lsp_range: multispy_types.Range, tree_range: tree_sitter.Range) -> bool:
    return (
        lsp_range["start"]["line"] == tree_range.start_point.row
        and lsp_range["end"]["line"] == tree_range.end_point.row
        and lsp_range["start"]["character"] == tree_range.start_point.column
        and lsp_range["end"]["character"] == tree_range.end_point.column
    )
...
with analyzer.start_server():
    ...
    query = Language.PYTHON.tree_sitter().query("""(identifier)@element""")
    ...
    for element in elements:
        ...
        print(
            f"{file_name}, [{element.range.start_point.row}, {element.range.start_point.column}] -> {def_file}, [{def_node.range.start_point.row}, {def_node.range.start_point.column}]"
        )
